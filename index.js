...
app.get("/edit-post/:id", (req, res, next) => {
  ...
  res.render("edit-post", { data: selectedItem })
})

app.put("/edit-post/:id", upload.single("image"), (req, res) => {
  let title = req.body.title
  let content = req.body.content // Corrected variable name
  let id = req.params.id
  const selectedItem = posts.find((item) => item.id == id)

  const imagePath = req.file ? req.file.path : selectedItem.image
  ...
  if (
    ...
    (req.file && selectedItem.image !== originalItem.image)
  ) {
